...
// and primitives (⚛️ indivisible, foundational — assumed background
...
function _buildConceptIndex(domain) {
  const index = new Map()
...
export function ContentPanel(domain, { level = 'intro', lang = 'en', graphViewer, signal } = {}) {
  ...
  const conceptIndex = _buildConceptIndex(domain)
    ...
    if (exists) {
      ...
      contentArea.innerHTML = `
        <div class="cb-ide-empty">
          ...
          <p>Not generated yet for <strong>${state.model || 'default'}</strong> / <strong>${state.level}</strong> / <strong>${state.lang}</strong>.</p>
